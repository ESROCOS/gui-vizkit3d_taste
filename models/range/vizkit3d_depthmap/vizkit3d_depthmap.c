...
#include "vizkit3d_taste/depthMapPluginWrapper.h"
#include <stdio.h>
...
void vizkit3d_depthmap_PI_updateDepthMap(const asn1SccBase_samples_DepthMap *IN_map)
{
    ...
    if (g_statusOk)
    {
        result = DepthMapVisualization_updateDepthMap(vizkit3d_depthmap_ctxt.id, IN_map);
...
void vizkit3d_depthmap_PI_updatePose(const asn1SccBase_samples_RigidBodyState *IN_pose)
{
    ...
    if (g_statusOk)
    {
        result = DepthMapVisualization_updatePose(vizkit3d_depthmap_ctxt.id, IN_pose);
